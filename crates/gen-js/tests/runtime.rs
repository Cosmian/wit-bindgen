...
fn execute(name: &str, wasm: &Path, ts: &Path, imports: &Path, exports: &Path) {
    ...
    let imports =
        cosmian_wit_bindgen_gen_core::cosmian_wit_parser::Interface::parse_file(imports).unwrap();
    let exports =
        cosmian_wit_bindgen_gen_core::cosmian_wit_parser::Interface::parse_file(exports).unwrap();
